...
- if you find an error, please notify us with a suggested correction and source at [correction [at] disdict [dot] org](mailto:correction@disdict.org)
- if you believe we are infringing your copyright please notify us with proof at [copyright [at] disdict [dot] org](mailto:copyright@disdict.org)
...
- the folders and filenames serve as target for html links, e.g. `https://disdict/intro/principles`
...
You can send plaintext suggestions for improvements, we do not accept binary files for security reasons. Please use the [quarto markdown](https://quarto.org/docs/authoring/markdown-basics.html) .qmd fileformat and send to [contribute [at] disdict [dot] org](mailto:contribute@disdict.org). The qmd syntax for defining section headers, footnotes and integrate pictures is simple, see for example the *part brothers | chapter russia* [in the dictionary](#sec-brothers-russia) and [on github](https://github.com/disinfodict/disinfodict/blob/main/brothers/russia.qmd). There is also a [mini demo dictionary](https://github.com/disinfodict/disinfoquick) for learning.  

For an overview which chapters still require work see [status](#sec-status-status). By submitting content, you accept the  [CC BY-NC-ND 4.0](https://creativecommons.org/licenses/by-nc-nd/4.0/) license. To avoid duplicate work, join our signal group and announce which chapter you are working on. To join, send your signal handle (or your mobile number) to [contribute [at] disdict [dot] org](mailto:contribute@disdict.org). Joining the group is invitation-only.  
