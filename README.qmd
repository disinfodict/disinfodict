...
Quarto seems not to set the meta name date: we write the last-modified date of the qmd file as publication date to the html file, then parse it, remove it from the html, set it as meta name date and store it for fixing sitemap.xml. Note that putting publication date into index.qmd causes a epub verification error "The content of the element “dc:date” is invalid; it must be a string with a minimum length of 1 (length is currently 0)", hence for the index files we skip this, i.e. index files inherit their meta name date from in_header.html 

```{R}
files <- dir("_book", "[.]html", recursive=TRUE, include.dirs = TRUE, full.names = TRUE)
dates <- vector("character", length(files))
names(dates) <- files
for (f in files){
  #f <- "_book/russia/history.html" 
  #f <- "_book/index.html" 
  b <- basename(f)
  lines <- readLines(f)
  i <- grep('<meta name="date" content="', lines)
  if (b == "index.html"){
    date <- sub('(^ *<meta name="date" content=")(.+)(">$)','\\2', lines[i])
    date <- format(as.POSIXct(date,'%Y-%m-%dT%H:%M:%S', tz=Sys.timezone()), '%Y-%m-%dT%H:%M:%S', tz="Europe/London")
  }else{
    p <- grep('<div class="quarto-title-meta-heading">', lines)
    d <- grep('<p class="date">', lines)
    stopifnot(d==p+2)  # check for changes or errors
    date <- sub('(^ *<p class="date">)(.+)(</p>$)','\\2',lines[d]) #<p class="date">2025-06-29T08:57:47</p>    
    date <- format(as.POSIXct(date,'%Y-%m-%dT%H:%M:%S', tz=Sys.timezone()), '%Y-%m-%dT%H:%M:%S', tz="Europe/London")
    lines[i] <- paste0('<meta name="date" content="', date,'">')
    writeLines(lines[(-(d+2)):(-(p-1))], f)  # write changed file removing published date
  }
  dates[f] <- date
}
# remove _book/
names(dates) <- substr(names(dates), 7, length(names(dates)))
```
