...
for (l in seq_along(ccs)){
  if (l==1){
    ...
    # remove all other sitemaps
    file.remove(file.path("_book", ccs[l], "sitemap.xml"))
    # remove from redireting index.html the erroneous lines with '<link rel="alternate" hreflang'
    index <- readLines(file.path("_book", ccs[l], "index.html"))
    writeLines(index[substr(index, 1, 30) != '<link rel="alternate" hreflang'], file.path("_book", ccs[l], "index.html"))
  }
}
```
...
ccs <- c("en","de")
# for secondary languages remove the redirection index.html and rename the country-specific one into index.html
for (cc in ccs[-1]){
...
for (f in files){
  ...
  for (cc in ccs){
    if (cc==ccs[1]){
      ...
      lines <- gsub(paste0('("', baseurl, '/', cc,  ')(/index[.]', cc , '[.]html")'), '\\1"', lines)
      lines <- gsub(paste0('("[^h][^"]*/',  cc, ')(/index[.]', cc, '[.]html")'), '\\1"', lines)
    }
  }
...
for (cc in ccs){
  if (cc==ccs[1]){
    ...
    lines <- gsub(paste0('(<loc>', baseurl, '/', cc, ')(/index[.]', cc, '[.]html</loc>)'), '\\1</loc>', lines)
    lines <- gsub(paste0('("', baseurl, '/', cc ,')(/index[.]', cc, '[.]html")'), '\\1"', lines)
  }
}
