...
- install TinyTex from the terminal in RStudio: quarto install tool tinytex
...
Here are some tips regarding performance. It's mostly about using .svg or well compressed images. 
...
```{R}
en <-  setdiff(dir(pattern="[.]qmd$", recursive=T), c("index.qmd", "index.de.qmd", "404.qmd", "README.qmd", "EXPERIMENT.qmd"))
...
```{R}
en <-  setdiff(dir(pattern="[.]qmd$", recursive=T), c("404.qmd", "README.qmd", "EXPERIMENT.qmd"))
de <- en[grep("[.]de[.]qmd$", en)]
ua <- en[grep("[.]ua[.]qmd$", en)]
en <- setdiff(en, c(de, ua))
demiss <- setdiff(sub("[.]qmd", ".de.qmd", en), de)
...
qmdfiles <- character()
for (cc in ccs[-1]){
  qmdfiles <- c(qmdfiles, sub(paste0('^', cc, '/'), '', sub('[.]html','.qmd', substr(htmlfiles, 7, nchar(htmlfiles)))))
}
...
```{R}
files <- setdiff(htmlfiles, paste0('_book/', c(htmlfilenames_onlydefault)))  # hold out rather technical pages
