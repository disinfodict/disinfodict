...
```{R}
en <-  setdiff(dir(pattern="[.]qmd$", recursive=T), c("impressum.qmd", "404.qmd", "README.qmd"))
...
- Advice what to do if sitemap can't be read: https://www.clickbrown.in/sitemap-could-not-be-read-cloudflare-fix/, note that Google does not like <whatever>.page.dev/sitemap.xml, not even - cloudflare likes it (fails to trace it with https://developers.cloudflare.com/rules/trace-request/how-to/). 
