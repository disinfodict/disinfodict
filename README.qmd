...
As of 2025-05-03 there were bugs in the sitemap setup of babelquarto (secondary language-files had wrong paths, there where multiple sitemap.xml but only one mentioned in robots.txt). The following replaces all those sitemap.xml with versions that list all available languages for each file. 
...
for (f in files){
  ...
  for (l in rev(seq_along(ccs))) {
    nn <- read_xml('<xhtml:link rel="alternate"/>', options="NOWARNING")  # suppresing the warnings "Namespace prefix xhtml on link is not defined" doesn't work
    cc <- ccs[l]
    xml_attr(nn, "hreflang") <- cc
    if (l == 1){
      xml_attr(nn, "href") <- def 
    } else {
      url <- paste0("https://", defcom$server, if (!is.na(defcom$port)) ":", defcom$port, "/", cc, sub("[.]", paste0(".", cc, "."), defcom$path))
      xml_attr(nn, "href") <- url 
    }
    xml_add_sibling(loc, nn)
  }
  tmp <- tempfile()
  for (l in rev(seq_along(ccs)[-1])){
    cc <- ccs[l]
    write_xml(f, tmp)
    fi <- read_xml(tmp)
...
for (l in seq_along(ccs)){
  write_xml(sitemap, (file.path(if (l==1) "_book" else file.path("_book", ccs[l]), "sitemap.xml")))
}
```
